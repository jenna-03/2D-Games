...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    m_light_intensity_uniform   = glGetUniformLocation(m_program_id, "lightIntensity");
    m_light_radius_uniform      = glGetUniformLocation(m_program_id, "lightRadius");
    m_tint_colour_uniform        = glGetUniformLocation(m_program_id, "tintColor");
    ...
    set_light_position(glm::vec2(0.0f, 0.0f));
    set_light_intensity(1.0f);
    set_light_radius(5.0f);
    
    glUseProgram(m_program_id);
    glUniform4f(m_tint_colour_uniform, 1.0f, 1.0f, 1.0f, 1.0f);
}
...
void ShaderProgram::set_light_intensity(float intensity)
{
    glUseProgram(m_program_id);
    glUniform1f(m_light_intensity_uniform, intensity);
}
...
void ShaderProgram::set_tint_color(float red, float green, float blue, float alpha) {
    glUseProgram(m_program_id);
    glUniform4f(m_tint_colour_uniform, red, green, blue, alpha);
}
